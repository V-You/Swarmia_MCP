...
|----------|-------|--------|-------|
| **High** | Command injection via `num_commits` in `_run_git()` | Explicitly cast `num_commits = int(num_commits)` with bounds checking (1–100) | |
| **High** | Arbitrary file read via `rglob("*.yml")` traversing entire filesystem | Scope to `.github/workflows/` or `.gitlab-ci.yml` only, limit traversal depth | |
| **High** | `app_name` injected unsanitized into YAML/Groovy templates | Sanitize `app_name` to alphanumeric + hyphens only | |
...
| **Low** | `workflow_name` user-controlled, injected into YAML `[{workflow_name}]` | Validate against a safe regex pattern (e.g. `[a-zA-Z0-9 _-]+`) | |
